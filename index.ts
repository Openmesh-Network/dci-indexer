...
import { ReservedStorage, Storage, UsersStorage, WaitlistedStorage } from "./types/storage.js";
import { watchReserved } from "./event-watchers/Reserved.js";
import { watchWaitlisted } from "./event-watchers/Waitlisted.js";
...
async function start() {
  ...
  const storage: Storage = {
    ...
    waitlisted: new PersistentJson<WaitlistedStorage>("waitlsited", []),
    users: new PersistentJson<UsersStorage>("users", {}),
  };

  multichainWatcher.forEach((contractWatcher) => {
    watchReserved(contractWatcher, storage);
    watchWaitlisted(contractWatcher, storage);
  });
  ...
  process.stdin.on("data", (input) => {
    try {
      ...
      if (command.startsWith("sync ")) {
        ...
        historySync(multichainWatcher, chainId, fromBlock, toBlock, [DCIReserveContract.address]).catch((err) =>
